...
// Autoswitch Button Props
type AutoSwitchButtonProps = {
    defaultValue: boolean,
    onEnable: () => void,
    onDisable: () => void
}

// Toggle to turn autoswitch on and off
const AutoSwitchButton:FC<AutoSwitchButtonProps> = (props) => {
    ...
    return (
        <div id={"toggle"}>
            <label htmlFor="toggle-autoswitch" className="inline-flex shrink-0 relative items-center cursor-pointer">
                <input type="checkbox" value="" id="toggle-autoswitch" className="sr-only peer" checked={enabled} onClick={() => setEnabled(!enabled)} readOnly/>
                ...
                <span className="ml-2 text-sm font-medium text-neutral-900 dark:text-teal-100">Auto Switch</span>
            </label>
        </div>
    )
...
export default AutoSwitchButton